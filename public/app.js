document.addEventListener("DOMContentLoaded", async function () {
  ...
  window.editExpense = async function (expenseId) {
    const expenseDetails = document.getElementById(
      `expense-${expenseId}-details`
    );
    const newName = prompt(
      "Enter new name:",
      expenseDetails.innerText.split(":")[0].trim()
    );
    const newAmount = prompt(
      "Enter new amount:",
      expenseDetails.innerText.split(":")[1].trim()
    );
  ...
  window.deleteExpense = async function (expenseId) {
    try {
      ...
      const expenseItem = document.getElementById(
        `expense-${expenseId}-details`
      );
      if (expenseItem) {
